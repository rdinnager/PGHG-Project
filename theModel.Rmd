...
colnames(prob_mat) <- colnames(Ar)
## plot probs
heatmap(prob_mat,  Rowv = as.dendrogram(as.hclust(ctree)), Colv = as.dendrogram(as.hclust(rtree)), labRow = ctree$tip.label, labCol = rtree$tip.label, scale="none", margins = c(5, 6), font.axis = 3, cexRow = 1, cexCol = 1, col = rev(grey(seq(0, 1, length = 50)))) 
```
