...
## plot probs
heatmap(prob_mat,  Rowv = as.dendrogram(as.hclust(ctree)), Colv = as.dendrogram(as.hclust(rtree)), labRow = ctree$tip.label, labCol = rtree$tip.label, scale="none", margins = c(5, 6), font.axis = 3, col = rev(grey(seq(0, 1, length = max(prob_mat)))), cexRow = 1, cexCol = 1, xlab = "Resource Species", ylab = "Consumer Species") 
```
